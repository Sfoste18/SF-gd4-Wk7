...
public class OptimUnit : MonoBehaviour
{
    ...
    void Update()
    {
        Profiler.BeginSample("Handling Time"); //begins profiling a piece of code with the custom ("..") label

        HandleTime();
        Profiler.EndSample(); //ends the current profiling sample

        Profiler.BeginSample("Rotating"); //Start of profiling
        var t = transform;
        ...
        Profiler.BeginSample("Moving"); //Start of profiling
        Move();
        Profiler.EndSample(); //End profiling

        Profiler.BeginSample("Boundry Check"); //Start of profiling
